...
cloudapps-cli の agent プロセスは `fork()` で生成されるため、親プロセスの環境変数をすべて継承します。`op run --env-file=.env` のようなシークレットマネージャー経由で起動すると、cloudapps-cli が必要としない他サービスのトークン（Slack, GitHub など）も agent プロセスの環境に載ります。
...
3. **最小権限の原則の違反**: agent が必要とするのは Microsoft Defender for Cloud Apps API の認証情報のみです。他サービスのトークンを保持する理由がありません。

### 制約

- `CLOUDAPPS_API_TOKEN` は `fork()` 前に `Config` 構造体に読み込み済みで、fork 後は環境変数を再参照しません。
...
- Config 構造体にコピーされた `api_token` はプロセスメモリに残ります（本 ADR のスコープ外）。
