...
pub fn handle(cmd: &CredentialsCommand) -> Result<(), AppError> {
    ...
    match cmd {
        ...
        CredentialsCommand::Delete { field, yes } => delete_value(&*store, *field, *yes),
...
fn delete_value(
    ...
    // Deletion is destructive and has no undo path (the Keychain
    // does not version entries). Default to prompting so a mistyped
    // command or an accidental history replay cannot silently drop
    // the user's token. `--yes` opts into the old silent behavior
    // for scripted use.
    if !skip_confirm {
        let prompt = format!(
            "Delete {} from the credential store? This cannot be undone. [y/N] ",
            field.key()
        );
    ...
    if saw_error {
        ...
        println!(
            "One or more entries could not be accessed. This usually means a \
             ...
             that changed the binary's code signature). For recovery steps see:"
        );
        println!("  https://github.com/hiboma/cloudapps-cli#notes-on-keychain-prompts");
    }
    Ok(())
}
...
/// Hand-written `Debug` so that a future `dbg!(scan)` / `{:?}` in a log
/// macro does not leak the plaintext secret carried by `Present`. The
/// `Unsupported` variant's static reason string is safe to print.
impl std::fmt::Debug for SecretScan {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SecretScan::Present(_) => f.write_str("Present(***)"),
            SecretScan::Absent => f.write_str("Absent"),
            SecretScan::Unsupported(reason) => f.debug_tuple("Unsupported").field(reason).finish(),
        }
    }
}
...
"#;
    ...
    #[test]
    fn secret_scan_debug_masks_present_value() {
        let scan = SecretScan::Present("super-secret-token".to_string());
    ...
    #[test]
    fn extract_rejects_escape_sequences() {
        // `api_token = "abc\"def"` — an escape sequence inside the basic
        // string. We refuse rather than silently mishandling the parse.
        let s = r#"api_token = "abc\"def""#;
        match extract_api_token(s) {
            SecretScan::Unsupported(_) => {}
            other => panic!("expected Unsupported (escape), got {:?}", other),
        }
    }

    #[test]
    fn extract_rejects_unterminated_string() {
        // `api_token = "abc` — no closing quote. Must be `Unsupported`,
        // not `Absent`: otherwise the later remove-line step would wipe
        // the line based on an unreliable scan.
        let s = "api_token = \"abc\n";
        match extract_api_token(s) {
            SecretScan::Unsupported(_) => {}
            other => panic!("expected Unsupported (unterminated), got {:?}", other),
        }
    }

    #[test]
    fn remove_preserves_crlf_line_endings() {
        // Files on Windows or generated by Windows tools may use CRLF.
        // The line-removal pass is driven by `split_inclusive('\n')`
        // which treats `\r\n` as the newline sequence transparently.
        let s = "[credentials]\r\napi_token = \"abc\"\r\napi_url = \"https://example.com\"\r\n";
        let out = remove_api_token_line(s);
        assert!(!out.contains("api_token"));
        assert!(out.contains("api_url = \"https://example.com\"\r\n"));
    }

    #[test]
    fn migrate_rollback_on_rewrite_failure() {
        // Prove the rollback invariant: if the toml rewrite fails AFTER
        // the Keychain write succeeded, the Keychain entry must be
        // deleted so the user is not left in a half-migrated state.
        //
        // We force the rewrite to fail by pointing `.cloudapps-credentials.toml`
        // at a read-only parent directory (owner-only mode 0o500 drops
        // write permission).
        use std::os::unix::fs::PermissionsExt;

        let dir = tempfile::tempdir().unwrap();
        let toml_path = dir.path().join(".cloudapps-credentials.toml");
        fs::write(&toml_path, "[credentials]\napi_token = \"abc\"\n").unwrap();

        // Chdir into the dir so find_credentials_toml() picks up the
        // cwd-relative file. fs::canonicalize will succeed on the file
        // itself even after the parent is made read-only.
        let saved_cwd = std::env::current_dir().unwrap();
        std::env::set_current_dir(dir.path()).unwrap();

        // Call the low-level step explicitly. We don't invoke `migrate()`
        // because it prompts on stdin; instead we reproduce its rollback
        // path.
        let store = crate::config::credential_store::MemoryStore::new();
        store.set(KEY_API_TOKEN, "abc").unwrap();

        // Flip the parent directory to read-only so the NamedTempFile
        // creation inside `atomic_replace` fails with EACCES. We keep
        // the restoration + cwd reset in scope guards so a panic does
        // not leave the test dir locked.
        let orig_perms = fs::metadata(dir.path()).unwrap().permissions();
        let mut ro = orig_perms.clone();
        ro.set_mode(0o500);
        fs::set_permissions(dir.path(), ro).unwrap();

        let rewrite_failed = atomic_replace(&toml_path, b"[credentials]\n").is_err();

        // Always restore so `tempdir` cleanup can run.
        fs::set_permissions(dir.path(), orig_perms).unwrap();
        std::env::set_current_dir(saved_cwd).unwrap();

        assert!(
            rewrite_failed,
            "atomic_replace should fail on read-only parent"
        );

        // Simulate the rollback the real migrate() runs.
        store.delete(KEY_API_TOKEN).unwrap();
        assert_eq!(
            store.get(KEY_API_TOKEN).unwrap(),
            None,
            "Keychain entry must be rolled back when the toml rewrite fails"
        );
    }
}
